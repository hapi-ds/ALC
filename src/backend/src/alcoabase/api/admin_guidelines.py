"""FastAPI router for AI regulatory guidelines generation endpoint.

Provides the POST /api/admin/generate-ai-guidelines endpoint that triggers
the full AI regulatory guidelines generation, upload, tagging, and workflow
...
References:
    ...
    - Requirements: 5.2, 5.6, 5.7, 5.8, 5.9
"""
...
router = APIRouter(prefix="/admin", tags=["Admin Guidelines"])


@router.post(
    "/generate-ai-guidelines",
    response_model=GuidelinesGenerationReport,
    responses={
        ...
        500: {
            "model": GuidelinesGenerationError,
            "description": "Guidelines generation failed",
        },
    },
)
async def generate_ai_guidelines(
...
) -> GuidelinesGenerationReport | JSONResponse:
    """Execute the AI regulatory guidelines generation and upload sequence.

    Triggers the full guidelines generation process: content generation
    for 4 documents (1 master + 3 sector-specific), document upload,
    ...
    Returns:
        GuidelinesGenerationReport on success (HTTP 200), or
        GuidelinesGenerationError on failure (HTTP 500), or
    ...
    try:
        ...
        logger.info(
            "AI guidelines generation completed successfully via API "
            "(user_id=%d, company_id=%d)",
            ctx.user_id,
            ctx.company_id,
            extra={"guidelines_step": "api_endpoint"},
        )
        ...
        if "already in progress" in error_msg.lower():
            logger.warning(
                "AI guidelines generation rejected — concurrent generation "
            ...
            return JSONResponse(
                status_code=409,
                content={
                    "detail": "Guidelines generation is already in progress. "
        ...
        logger.error(
            "AI guidelines generation failed via API: %s",
            error_msg,
            exc_info=True,
            extra={"guidelines_step": "api_endpoint"},
        )
        ...
        generation_error = GuidelinesGenerationError(
            error=f"AI guidelines generation failed: {error_msg}",
        ...
        error_msg = str(e)
        logger.error(
            "AI guidelines generation failed via API (unexpected): %s",
            error_msg,
            exc_info=True,
            extra={"guidelines_step": "api_endpoint"},
        )
...
def _determine_failed_operation(error_msg: str) -> str:
    ...
    if "no ai task types" in msg_lower or "risk" in msg_lower:
        return "risk_data_load"
    if (
...
def _extract_document_title(error_msg: str) -> str | None:
    ...
    # "Content validation failed for 'AlcoaBase — AI Usage Guidelines (Pharma / GMP)'"
