...
* 👁️ **Document Content Viewer & Download**
  Inline document viewing directly on the document detail page — PDFs render natively in-browser, Markdown is displayed as formatted HTML, and unsupported formats offer a fallback download. A dedicated download/preview API streams file bytes from MinIO with correct MIME types, content-length headers, and RBAC enforcement. Download buttons appear across all application surfaces (search results, knowledge chat citations, signature pages). Version-aware viewing lets users switch between document versions seamlessly. All download and preview access events are logged immutably to the audit trail.
...
| — | Document Content Viewer & Download | 🔲 Planned |
